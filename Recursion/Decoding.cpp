...
int generateDecodings(char* inp, char* out, int i, int j) {
	...
	if(digit!=0){
		...
		// A.4 recursion
	count=	generateDecodings(inp, out, i+1, j+1);
		...
		if(inp[i+1]!='\0') {
			...
			if(number<=26) {
				...
				// B.5 recusion
			count+=	generateDecodings(inp, out, i+2, j+1);
			}
		}
	}
...
cout<<	generateDecodings(inp, out, 0, 0);
